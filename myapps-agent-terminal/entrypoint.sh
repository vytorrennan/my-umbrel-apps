...
# Start ttyd serving Herdr interactive terminal as developer user with modern theme & Nerd Fonts
echo "Starting Agent Terminal on port 7681..."
exec su - developer -c 'export TERM=xterm-256color; exec ttyd -p 7681 -W -i 0.0.0.0 \
  -t fontSize=15 \
  -t fontFamily="\"FiraCode Nerd Font\", \"JetBrainsMono Nerd Font\", \"Fira Code\", \"JetBrains Mono\", monospace" \
  -t cursorBlink=true \
  -t cursorStyle=bar \
